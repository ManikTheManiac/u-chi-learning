...
class LogULearner:
    def __init__(self, 
        ...
        self.new_theta = torch.Tensor([-1]).to(self.device)
    ...
    def _initialize_networks(self):
        ...
        self.theta_net = ThetaNet(self.env, hidden_dim=self.hidden_dim, device=self.device)

        # Make LogU learnable:
        self.optimizer = torch.optim.Adam(self.online_logu.parameters(), lr=self.learning_rate)
        self.theta_optimizer = torch.optim.Adam(self.theta_net.parameters(), lr=self.learning_rate)
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=1000, gamma=0.985)
    ...
    def learn(self,):
        ...
        for grad_step in range(self.gradient_steps):
            ...
            with torch.no_grad():
                ...
                # self.new_theta = self.ref_reward - torch.log(ref_chi)

                expected_curr_logu = self.beta * (rewards + theta) + \
                      (1 - dones) * next_logu#next_chi)# / ref_chi)
                # self.theta = torch.clamp(new_theta, 0, 1)
                # self.theta = (1 - self.tau_theta)*self.theta + self.tau_theta*self.new_theta
            ...
            # theta loss:
            theta_loss = F.smooth_l1_loss(theta, self.theta * torch.ones_like(theta))
            self.logger.record("theta_loss", theta_loss.item())
            self.theta_optimizer.zero_grad()
            theta_loss.backward()            

            self.optimizer.zero_grad()
            self.logger.record("lr", self.scheduler.get_last_lr()[0])
            # Step the optimizer schedule:
            self.optimizer.step()
            self.scheduler.step()
        ...
        # interact with the environment, episodically:
            ...
            while not done:
                ...
                if self.env_steps % self.target_update_interval == 0:
                    ...
                    # update theta:
                    # self.theta = (1 - self.tau_theta)*self.theta + self.tau_theta*self.new_theta
                    # self.theta = 0.1*self.new_theta + 0.9*self.theta
...
def main():
    ...
    agent = LogULearner(env, **config, log_interval=100, hidden_dim=256)
